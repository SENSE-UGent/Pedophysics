...
def test_sample_C0():
        ...
        assert BulkEC(sample_C0) == np.array([0.0072, 0.007,  0.0075, 0.008])
...
def test_sample_C0c():
        ...
        #print("BulkEC(sample_C0c)", BulkEC(sample_C0c))
...
def test_sample_C0d():
        ...
        #print("BulkEC(sample_C0d)", BulkEC(sample_C0d))
...
def test_sample_C1b():
        ...
        expected_result = np.array([0.006, 0.011, 0.009, 0.011636, 0.000123, np.nan, 0.008, 0.0085]) 
        assert arrays_are_similar(BulkEC(sample_C1b), expected_result)   
        sample_C1b.info.to_excel('sample_C1b_info.xlsx')
        #sampleC1b.df.to_excel('sampleC1b_df.xlsx')
        fig = plt.figure()
        ax2e = fig.add_subplot(1, 1, 1)
        aa = 0.5
        ss = 50
        ax2e.set_title('Example C1b')
        ax2e.scatter(sample_C1b.df.water, sample_C1b.bulk_ec, marker='D', color='black', s=ss)
        #ax2e.plot(sample_C1b.df.water, BulkEC(sample_C1b), 'ro', alpha=aa, markersize=8)
        plt.show()
test_sample_C1b()
...
def test_sample_C1c():
        ...
        assert arrays_are_similar(BulkEC(sample_C1c), expected_result)                                              # [  0.002  0.055   0.009393 0.012327 0.01522  nan     0.012   0.022389 0.011016]
...
# In this example, the solution by fitting is possible thanks to the prediction of water_ec without additional information.
        ...
        sample4_Rhoades = Rhoades(sample_C4.df.water, 0.0264, 0.00628, 1.0, 0.38)
