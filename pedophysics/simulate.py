...
class Soil(object):
    ...
    def __init__(
            ...
            bulk_cond = np.nan, bulk_icond = np.nan, water_cond = np.nan, solid_cond = np.nan, air_cond = np.nan,
            dry_cond = np.nan, sat_cond = np.nan, L = np.nan, Lw = np.nan, m = np.nan, n = np.nan, alpha = np.nan, 
            frequency_perm = np.nan, frequency_cond = np.nan,
            land = np.nan, region = np.nan, depth = np.nan, texture = np.nan, instrument = np.nan, pH_water = np.nan, df = np.nan, roundn=2):
        ...
        """
            ...
            dry_cond: float
                Soil real electrical conductivity at zero water content [S/m]    

            sat_cond: float
                Soil real electrical conductivity at saturation water content [S/m]    
        ...
        assert (type(air_perm) == float) or (type(air_perm) == int) or (air_perm == np.nan), "'air_perm' must be an int, float or np.nan"
        ...
        assert (type(solid_cond) == float) or (type(solid_cond) == np.ndarray) or (type(solid_cond) == int) or (solid_cond == np.nan), "'solid_cond' must be an int, float, numpy.array or np.nan"
        assert (type(air_cond) == float) or (type(air_cond) == int) or (air_cond == np.nan), "'air_cond' must be an int, float, numpy.array or np.nan"
        assert (type(dry_cond) == float) or (type(dry_cond) == np.ndarray) or (type(dry_cond) == int) or (dry_cond == np.nan), "'dry_cond' must be an int, float, numpy.array or np.nan"
        assert (type(sat_cond) == float) or (type(sat_cond) == np.ndarray) or (type(sat_cond) == int) or (sat_cond == np.nan), "'sat_cond' must be an int, float, numpy.array or np.nan"      
        ...
        assert (type(alpha) == float) or (type(alpha) == int) or (alpha == np.nan), "'alpha' must be an int, float, numpy.array or np.nan"
        ...
        assert (type(roundn) == int), "'roundn' must be an int"
...
### Here we define the state variables
        ...
        self.solid_perm = solid_perm                      # Soil phase real relative dielectric permittivity [-]
        if type(self.solid_perm) != np.ndarray:
            self.solid_perm = np.array([solid_perm]) 
        ...
        self.solid_cond = solid_cond                        # Soil solid real electrical conductivity [S/m]
        if type(self.solid_cond) != np.ndarray:
            self.solid_cond = np.array([solid_cond])  

        self.dry_cond = dry_cond                        # Soil real electrical conductivity at zero water content [S/m]
        if type(self.dry_cond) != np.ndarray:
            self.dry_cond = np.array([dry_cond])  

        self.sat_cond = sat_cond                        # Soil real electrical conductivity at saturation water content [S/m]
        if type(self.sat_cond) != np.ndarray:
            self.sat_cond = np.array([sat_cond])  
...
### Here we define non-state variables
        ...
        len(self.solid_perm), len(self.water_iperm), len(self.bulk_cond), len(self.bulk_icond), 
        len(self.water_cond), len(self.solid_cond), len(self.dry_cond), len(self.sat_cond),
...
### Fill the state variables with nans when are shorter than n_states
        ...
        if len(self.solid_perm) != n_states:
            self.solid_perm  = np.append(self.solid_perm , [np.nan]*(n_states - len(self.solid_perm )))  
        ...
        if len(self.solid_cond  ) != n_states:
            self.solid_cond   = np.append(self.solid_cond  , [np.nan]*(n_states - len(self.solid_cond ))) 

        if len(self.dry_cond  ) != n_states:
            self.dry_cond   = np.append(self.dry_cond  , [np.nan]*(n_states - len(self.dry_cond ))) 

        if len(self.sat_cond  ) != n_states:
            self.sat_cond   = np.append(self.sat_cond  , [np.nan]*(n_states - len(self.sat_cond ))) 
...
### If just the state zero is defined in a state variable, then fully fill it.
        if (self.temperature[0] != np.nan) and (np.isnan(self.temperature[1:(n_states)])).all():
            self.temperature[1:n_states] = self.temperature[0] 

        if (self.water[0] != np.nan) and (np.isnan(self.water[1:(n_states)])).all():
            self.water[1:n_states] = self.water[0] 

        if (self.bound_water[0] != np.nan) and (np.isnan(self.bound_water[1:(n_states)])).all():
            self.bound_water[1:n_states] = self.bound_water[0] 

        if (self.salinity[0] != np.nan) and (np.isnan(self.salinity[1:(n_states)])).all():
            self.salinity[1:n_states] = self.salinity[0] 

        if (self.sand[0] != np.nan) and (np.isnan(self.sand[1:(n_states)])).all():
            self.sand[1:n_states] = self.sand[0] 

        if (self.silt[0] != np.nan) and (np.isnan(self.silt[1:(n_states)])).all():
            self.silt[1:n_states] = self.silt[0] 

        if (self.clay[0] != np.nan) and (np.isnan(self.clay[1:(n_states)])).all():
        ...
        if (self.CEC[0] != np.nan) and (np.isnan(self.CEC[1:(n_states)])).all():
            self.CEC[1:n_states] = self.CEC[0] 

        if (self.LOI[0] != np.nan) and (np.isnan(self.LOI[1:(n_states)])).all():
            self.LOI[1:n_states] = self.LOI[0] 

        if (self.OC[0] != np.nan) and (np.isnan(self.OC[1:(n_states)])).all():
            self.OC[1:n_states] = self.OC[0] 

        if (self.orgm[0] != np.nan) and (np.isnan(self.orgm[1:(n_states)])).all():
        ...
        if (self.solid_perm[0] != np.nan) and (np.isnan(self.solid_perm[1:(n_states)])).all():
            self.solid_perm[1:n_states] = self.solid_perm[0] 
        ...
        if (self.solid_cond[0] != np.nan) and (np.isnan(self.solid_cond[1:(n_states)])).all():
            self.solid_cond[1:n_states] = self.solid_cond[0]

        if (self.dry_cond[0] != np.nan) and (np.isnan(self.dry_cond[1:(n_states)])).all():
            self.dry_cond[1:n_states] = self.dry_cond[0]

        if (self.sat_cond[0] != np.nan) and (np.isnan(self.sat_cond[1:(n_states)])).all():
            self.sat_cond[1:n_states] = self.sat_cond[0]
...
### Defining Soil.df  
    ...
    def get_solid_cond(self):
        return self.solid_cond
    def get_air_cond(self):
        return self.air_cond
    def get_dry_cond(self):
        return self.dry_cond
    def get_sat_cond(self):
        return self.sat_cond
    def get_L(self):
        return self.L
    def get_Lw(self):
        return self.Lw
    ...
    def set_solid_cond(self, new_solid_cond):
        self.solid_cond = new_solid_cond
    def set_air_cond(self, new_air_cond):
        self.air_cond = new_air_cond
    def set_dry_cond(self, new_dry_cond):
        self.dry_cond = new_dry_cond
    def set_sat_cond(self, new_sat_cond):
        self.sat_cond = new_sat_cond
