...
def WaterEC(soil):
    ...
    if sum(not np.isnan(soil.df.bulk_ec[x]) and (not np.isnan(soil.df.water[x]) or not np.isnan(soil.df.bulk_perm[x])) and np.isnan(soil.df.water_ec[x]) for x in range(soil.n_states)) >= 2:
        fitting(soil)
...
def from_salinity(soil):
    ...
    soil.info['water_ec'] = ["Calculated using SenGood function in predict.water_ec.from_salinity" if np.isnan(soil.df.water_ec[x]) or soil.info.water_ec[x] == "Calculated using SenGood function in predict.water_ec.from_salinity"
...
def from_bulk_ec(soil):
    ...
    soil.info['water_ec'] = ["Calculated using Fu function in predict.water_ec.from_bulk_ec" if np.isnan(soil.df.water_ec[x]) or soil.info.water_ec[x] == "Calculated using Fu function in predict.water_ec.from_bulk_ec"
                                 else soil.info.water_ec[x] for x in range(soil.n_states)]

    soil.df['water_ec'] = [round(wat_ec[x], soil.roundn+3) if np.isnan(soil.df.water_ec[x]) else soil.df.water_ec[x] for x in range(soil.n_states) ]
...
def fitting_rhoades(soil):
    ...
    soil.info['s_ec'] = ["Calculated by fitting Rhoades function in predict.water_ec.fitting_rhoades" if np.isnan(soil.df.s_ec[x]) & valid_indices[x]
                            or soil.info.s_ec[x] == "Calculated by fitting Rhoades function in predict.water_ec.fitting_rhoades"
                            else soil.info.s_ec[x] for x in range(soil.n_states)]
    
    soil.df['s_ec'] = [round(best_s_ecs, soil.roundn+3) if np.isnan(soil.df.s_ec[x]) & valid_indices[x] else soil.df.s_ec[x] for x in range(soil.n_states) ]
    ...
    soil.info['water_ec'] = ["Calculated by fitting (R2 = "+str(R2)+") Rhoades function in predict.water_ec.fitting_rhoades" if np.isnan(soil.df.water_ec[x]) & valid_indices[x]
                            or soil.info.water_ec[x] == "Calculated by fitting (R2 = "+str(R2)+") Rhoades function in predict.water_ec.fitting_rhoades"
                            else soil.info.water_ec[x] for x in range(soil.n_states)]

    soil.df['water_ec'] = [round(best_water_ec, soil.roundn+3) if np.isnan(soil.df.water_ec[x]) & valid_indices[x] else soil.df.water_ec[x] for x in range(soil.n_states) ]
...
def fitting_hilhorst(soil):
    ...
    soil.info['offset_perm'] = ["Calculated by fitting Hilhorst function in predict.water_ec.fitting_hilhorst" if np.isnan(soil.df.offset_perm[x]) & valid_indices[x] or soil.info.offset_perm[x] == "Calculated by fitting Hilhorst function in predict.water_ec.fitting_hilhorst"
                                 else soil.info.offset_perm[x] for x in range(soil.n_states)]
    
    soil.df['offset_perm'] = [round(best_offset_perm, soil.roundn+3) if np.isnan(soil.df.offset_perm[x]) & valid_indices[x] else soil.df.offset_perm[x] for x in range(soil.n_states) ]

    # Calculating the R2 score of the fitting
    R2 = round(R2_score(arg_bulk_perm, Hilhorst(arg_EC, best_water_ec, arg_water_perm, best_offset_perm)), soil.roundn)
    
    # Saving calculated water_ec and its info with R2
    soil.info['water_ec'] = ["Calculated by fitting (R2="+str(R2)+") Hilhorst function in predict.water_ec.fitting_hilhorst" if np.isnan(soil.df.water_ec[x]) & valid_indices[x] or soil.info.water_ec[x] == "Calculated by fitting (R2="+str(R2)+") Hilhorst function in predict.water_ec.fitting_hilhorst"
                                 else soil.info.water_ec[x] for x in range(soil.n_states)]
